...
vec3 sulfurSky(vec3 viewDir, vec3 up, vec3 sunP, vec3 moonP, vec3 vanillaSky, vec3 vanillaFog) {
	...
	float sunGlow = pow(sunDot, 36.0) * 0.38 * day;
	float sunWash = pow(sunDot, 12.0) * 0.07 * day;
	sky += vec3(1.00, 0.84, 0.52) * sunGlow;
	sky += vec3(1.00, 0.72, 0.40) * sunWash * duskAmt;

	vec3 moonDir = normalize(moonP);
	float moonDot = max(dot(viewDir, moonDir), 0.0);
	float nightAmt = 1.0 - day;
	float moonHalo = pow(moonDot, 28.0) * 0.42;
	float moonBloom = pow(moonDot, 8.0) * 0.16;
	sky += vec3(0.62, 0.70, 0.92) * (moonHalo + moonBloom) * nightAmt;
