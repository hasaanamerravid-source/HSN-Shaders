...
vec3 sulfurSky(vec3 viewDir, vec3 up, vec3 sunP, vec3 moonP, vec3 vanillaSky, vec3 vanillaFog) {
	...
	// No drawn sun/moon disc — vanilla gbuffers_skytextured keeps the real sprites.
	vec3 sunDir = normalize(sunP);
	float sunDot = max(dot(viewDir, sunDir), 0.0);
	float sunGlow = pow(sunDot, 64.0 / max(SUN_SIZE, 0.25)) * 0.22 * day;
	sky += vec3(1.00, 0.82, 0.52) * sunGlow;

	vec3 moonDir = normalize(moonP);
	float moonDot = max(dot(viewDir, moonDir), 0.0);
	float nightAmt = 1.0 - day;
	float moonHalo = pow(moonDot, 110.0 / max(MOON_SIZE, 0.25)) * 0.12;
	sky += vec3(0.55, 0.65, 0.90) * moonHalo * nightAmt;
